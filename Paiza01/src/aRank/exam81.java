...
public class exam81 {
	...
	public static void main(String[] args) throws IOException {
		...
		List<Integer> tlst = new ArrayList<>(); // node1, node3, node4, ... 
		...
		for(int i=0; i<m; i++) {
			...
			if (!tlst.contains(a)) tlst.add(a);
			if (!tlst.contains(b)) tlst.add(b);
			if(min > Math.min(a, b)) {
				min = Math.min(a, b);
				minIdx = i;
			}
...
//		}
		...
		for(int i=0; i<g.length; i++) {
			int t1 = Math.min(g[i][0], g[i][1]);
			int t2 = Math.max(g[i][0], g[i][1]);
			
			if(!dictionary.containsKey(t1)) {
				List<Integer> tmp = new ArrayList<>();
				tmp.add(t2);
				dictionary.put(t1, tmp);
			}else {
				dictionary.get(t1).add(t2);
			}
		}
		
		for(int i : tlst) {
			List<Integer> tmp = dictionary.get(i);
			if(tmp != null) {
